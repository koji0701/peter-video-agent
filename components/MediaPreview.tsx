...
export const MediaPreview: React.FC<MediaPreviewProps> = ({ scriptLines, videoUrl, audioUrl, audioStatusMessage }) => {
  const hasMedia = videoUrl || audioUrl || audioStatusMessage;
  const hasContentToDownload = scriptLines.length > 0 || audioUrl || videoUrl;
  ...
  if (!hasMedia && scriptLines.length === 0) {
    return (
      <section className="p-6 bg-gray-800 rounded-xl shadow-2xl text-center">
        ...
        <p className="text-gray-500">Generate content to see the preview and download options here.</p>
      </section>
    );
  }
      ...
      {hasMedia && (
            ...
            {(audioUrl || audioStatusMessage) && (
              <div className="p-4 bg-gray-700 rounded-lg shadow">
                ...
                {audioUrl && (
                 ...
                 {!audioUrl && !audioStatusMessage && (
                  <p className="text-gray-500">No audio information available.</p>
                )}
              </div>
            )}
          </div>
       ...
       {hasMedia && videoUrl && (
         <p className="text-xs text-gray-500 mt-[-1.5rem] mb-6 text-center">
            Background video is muted. The generated audio should be played over this video.
         </p>
       )}
      ...
      {hasContentToDownload && (
        <div className="mt-6 pt-6 border-t border-gray-700">
          ...
          <div className="space-y-3 md:space-y-0 md:flex md:space-x-4">
            ...
            {videoUrl && (
              <a
                href={videoUrl.replace("shorts/", "watch?v=").split('&')[0]} // Try to get a more standard watch link for YouTube
                target="_blank"
                rel="noopener noreferrer"
          ...
          <p className="text-xs text-gray-400 mt-4">
            ...
            Directly merging media in the browser is highly complex and typically a server-side task.
          </p>
        </div>
      )}
    </section>
